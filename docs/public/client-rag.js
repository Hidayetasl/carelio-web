...
function score(q, text){
  ...
  const ts = norm(text);
  let s=0; for(const w of qs){ if(w.length>2 && ts.includes(w)) s++; }
  return s;
}

export function answerWithRAG(q, kb, chunks){
  let bestKB = kb.map(it=>({it, s: score(q, it.q + ' ' + (it.tags||[]).join(' ') + ' ' + it.a)})).sort((a,b)=>b.s-a.s).slice(0,2);
  if(bestKB[0]?.s>=2){ return { type:'kb', text: bestKB[0].it.a }; }
  let bestCH = chunks.map(it=>({it, s: score(q, (it.text||'') + ' ' + (it.keywords||[]).join(' '))})).sort((a,b)=>b.s-a.s)[0];
